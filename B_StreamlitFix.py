...
def load_models():
    ...
    try:
        ...
        #st.sidebar.write("Fitur yang dimuat dari 'feature_columns.pkl':", len(feature_names))
        #st.sidebar.write(feature_names) # Aktifkan ini untuk melihat daftar fitur lengkap
...
def prepare_input_data(feature_names, **kwargs):
    ...
    feature_mapping = {
        ...
        # KOREKSI: Tambahkan TARIF_RS yang hilang
        'TARIF_RS': float(kwargs.get('total_tarif', 0)) # Asumsi TARIF_RS sama dengan TOTAL_TARIF
    }
...
def create_system_performance_chart():
    ...
    values = [85.25, 85.67, 97.94, 91.39]
...
def create_monthly_prediction_chart():
    """Create monthly prediction statistics chart"""
    months = ['Jan', 'Feb', 'Mar', 'Apr', 'Mei', 'Jun']
    accepted = [245, 289, 312, 298, 334, 356]
    rejected = [89, 76, 92, 87, 78, 84]
    ...
    fig.update_layout(
        title="Statistik Prediksi Bulanan",
        xaxis_title="Bulan",
...
def show_system_info_tab():
    ...
    with col_chart1:
        ...
        performance_chart = create_system_performance_chart()
        st.plotly_chart(performance_chart, use_container_width=True)
    
    with col_chart2:
        st.subheader("📊 Statistik Bulanan")
        monthly_chart = create_monthly_prediction_chart()
        st.plotly_chart(monthly_chart, use_container_width=True)
    
    # Feature importance
    st.subheader("🔍 Fitur Penting dalam Prediksi")
    
    # Menggunakan list kategori_fitur_lengkap yang sudah didefinisikan secara global
    feature_importance_data = {
        ...
        'Kategori': kategori_fitur_lengkap # Menggunakan list yang sudah lengkap
    }
    
    # KOREKSI: Buat DataFrame terlebih dahulu, baru kemudian urutkan
    feature_importance_df = pd.DataFrame(feature_importance_data).sort_values(
        'Tingkat Kepentingan', ascending=True # Sortir ascending agar fitur terpenting di atas
    )
    
    fig_importance = px.bar(
        feature_importance_df, # Gunakan DataFrame yang sudah diurutkan di sini
        x='Tingkat Kepentingan',
        y='Fitur',
        color='Kategori', # Kembali aktifkan pewarnaan berdasarkan kategori
        orientation='h',
    ...
    fig_importance.update_layout(height=800) # Pastikan tingginya cukup
...
def show_user_input_tab():
    ...
    with col_input:
        ...
        with st.expander("👤 Data Dasar Pasien", expanded=True):
            ...
            # Menambahkan input untuk Umur_Hari
    ...
    with col_result:
        ...
        if predict_button:
            ...
            input_df = prepare_input_data(
                ...
                total_tarif=total_tarif, # Ini akan digunakan juga untuk TARIF_RS
                los=los,
                ...
                obat_kemo=obat_kemo
                # ADL1_12 tidak ada input UI, akan tetap 0 dari inisialisasi awal
            )
            ...
            try:
                ...
                if status_pred == 1:
                    ...
                    fig_comparison = px.bar(
                        ...
                        title="Perbandingan Nilai Tarif"
                    )
                    ...
                    # Logika ini bisa disempurnakan berdasarkan feature importance
                    ...
                    if prosedur_bedah > tarif_inacbg: # Ini mungkin terlalu menyederhanakan
                ...
                # Probability chart (tampilkan di kedua kasus, diterima atau ditolak)
                ...
                st.warning("Pastikan nilai input numerik tidak kosong atau NaN. Jika error berlanjut, cek konsol Streamlit untuk detail.")
                with st.expander("Debug Informasi (klik untuk melihat detail error)"):
                    st.write(f"Input DataFrame shape: {input_df.shape}")
                    st.write(f"Expected feature names length: {len(feature_names)}")
                    st.write(f"Input data columns: {input_df.columns.tolist()}") # Tambahkan ini untuk cek kolom
                    st.write("Input data preview:")
                    st.dataframe(input_df) # Cetak seluruh DataFrame
                    st.write("Detail error:", e)
                    st.stop() # Hentikan eksekusi setelah error untuk memudahkan debugging
        
        else:
            # Welcome message for input tab
            st.markdown("#### 🎯 Panduan Input Data")
            st.markdown("Silakan lengkapi formulir di sebelah kiri untuk mendapatkan prediksi klaim BPJS.")
            
            st.markdown("##### 📝 Tips Pengisian:")
            st.markdown("""
            - Pastikan semua data akurat dan sesuai dengan dokumen medis.
            - Isi rincian biaya perawatan untuk analisis yang lebih detail.
            - Periksa kembali data sebelum melakukan prediksi.
            """)

def main():
    # Header dengan animasi
    st.markdown('<h1 class="main-header">🏥 Sistem Prediksi Klaim Asuransi BPJS</h1>', unsafe_allow_html=True)
    st.markdown('<p style="text-align: center; font-size: 1.2rem; color: #666; margin-bottom: 3rem;">Prediksi Status dan Estimasi Jumlah Klaim dengan Machine Learning</p>', unsafe_allow_html=True)
    ...
    <div style="text-align: center; color: #666; padding: 2rem;">
        <p>🏥 <strong>Sistem Prediksi Klaim BPJS</strong> | Powered by Random Forest Algorithm</p>
    </div>
    """, unsafe_allow_html=True)
